...
void WebAdminParaConfig::registerPropert(const std::string &name, ParameterRefType value)
{
...
void WebAdminParaConfig::registerPropert(const std::string &name, ParameterRefType value, std::function<void (ParameterType)> setCaller)
{
    auto p = new WebRegisterPara;
    p->name = name;
    p->value = value;
...
void WebAdminParaConfig::removePropert(const std::string &name)
{
    list.erase(name);
    updateVersion++;
}

std::map<std::string, WebRegisterPara *> &WebAdminParaConfig::getPropertList() {
    return list;
}

int WebAdminParaConfig::getVersion()
{
...
WebAdminParaConfig::WebAdminParaConfig()
{
    updateVersion = 0;
}
...
{
    if(currentVersion < WebAdminParaConfig::getInstance().getVersion())
    {
        currentVersion = WebAdminParaConfig::getInstance().getVersion();
        clenaAllWidget();
        //copy avoid thread race
        std::map<std::string, WebRegisterPara*>  list =  WebAdminParaConfig::getInstance().getPropertList();
...
std::string WebAdminServer::getCurrentValString(WebRegisterPara *regPar)
{
    ...
    if(regPar->value.type() == typeid (int*))
    {
        ValString = std::to_string(*boost::get<int*>(regPar->value));
    }
...
{
    ParameterType ret;
    if(setValue.type() == typeid (int*) ||  setValue.type() == typeid (uint64_t*) || setValue.type() == typeid (uint32_t*) )
    {
        ret = std::stoi(newValStr);
    }
